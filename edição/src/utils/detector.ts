    ...
    video.onloadedmetadata = async () => {
      try {
          ...
          await new Promise<void>((resolveSeek) => {
            const onSeeked = () => {
              ...
              if (i === Math.floor(numSamples / 3)) {
                // Draw at larger resolution for beautiful preview card
                const thumbCanvas = document.createElement('canvas');
                thumbCanvas.width = 360;
                thumbCanvas.height = 640;
                const thumbCtx = thumbCanvas.getContext('2d');
                if (thumbCtx) {
                  thumbCtx.drawImage(video, 0, 0, 360, 640);
        ...
        if (activePixelCount > 50 && maxX > minX && maxY > minY) {
          // Add small safety margin padding
          const margin = 2;
          const finalX = Math.max(0, Math.round((minX - margin) * ratioX));
          const finalY = Math.max(0, Math.round((minY - margin) * ratioY));
          const finalW = Math.min(width - finalX, Math.round((maxX - minX + margin * 2) * ratioX));
          const finalH = Math.min(height - finalY, Math.round((maxY - minY + margin * 2) * ratioY));
